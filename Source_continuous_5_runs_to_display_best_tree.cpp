...
int main()
{
	...
	double accuracy, tree1Accuracy = 0, tree2Accuracy = 0;
	int L, K;
	vector< vector<double> > source;
	srand(time(NULL));
	node * bestTree1 = NULL, *bestTree2 = NULL;
	...
	for (int i = 1; i <= 5; i++)
	{
		...
		myfile << "Value of L: " << L << endl << "Value of K: " << K << endl;
		...
		accuracy = accuracyCalulator(source, head);
		if (tree1Accuracy < accuracy)
		{
			tree1Accuracy = accuracy;
			bestTree1 = head;
		}
		...
		decisionTreeBuilder(source, 0, head2);

		source.clear();
		readFile(source, "2//test_set.csv");
		accuracy = accuracyCalulator(source, head2);
		cout << "\n\nAccuracy for test dataset2  before pruning = " << accuracy << "%" << endl;
		myfile << "\n\nAccuracy for test dataset2 before pruning = " << accuracy << "%" << endl << endl;

		head2 = postPruning(L, K, head2, "2//validation_set.csv");
		accuracy = accuracyCalulator(source, head2);
		if (tree2Accuracy < accuracy)
		{
			tree2Accuracy = accuracy;
			bestTree2 = head2;
		}
		...
		source.clear();
		cout << "\nDo you wanna break ? Press 'y' or 'n'." << endl;
		char repeat;
		cin >> repeat;
		if (repeat == 'y')
			break;
			...
			cout << "\nInvalid choise. Continueing with the loop." << endl;
	}
	myfile << "\nThe best tree structure for the first data set post pruning: " << endl;
	printTree(bestTree1, "");

	myfile << "\n\nThe best tree structure for the second data set post pruning: " << endl;
	printTree(bestTree2, "");
